...
| runtime_thread_stack_size | Stack size in bytes for every thread a service runtime or an offload pool starts. A worker polls the whole application future chain on this stack, and a debug build gives every async state machine far larger frames than a release build. Default: `8388608` (8 MiB, the `RLIMIT_STACK` default Linux gives a main thread). Raising it costs reserved address space and not resident memory; it also applies to blocking pools, whose default cap is 512 threads per runtime, so set `max_blocking_threads` alongside it under `RLIMIT_AS` or a virtual-memory cgroup limit. | number |
